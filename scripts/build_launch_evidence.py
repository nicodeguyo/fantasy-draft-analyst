...
def main():
    ...
    claim = f'Archived v2 benchmark (does not evaluate v3): across {n:,} simulated drafts, the saved-board policy produced {gain} {direction} projected starting-lineup points than the noisy ADP-based draft bot, on average.'
    ...
    fragment = f'''<section class="evidence"><div class="wrap evidence-layout"><div><p class="eyebrow">Internal simulation · reproducible results</p><h2>A model you can<br>check yourself.</h2><p>{html.escape(claim)}</p><p class="caption">Metric: {metric}. Uses the same saved projections and opponent model. This is not a real-world backtest or a comparison with another draft assistant.</p><a class="text-link" href="https://github.com/nicodeguyo/fantasy-draft-analyst/blob/main/docs/archive/v2-benchmark.md#reproduce-the-historical-comparison">Reproduce the benchmark →</a></div><div class="benchmark" aria-label="Mean projected starting-lineup totals across {n} drafts">{benchmark_rows}<p class="caption">Paired Monte Carlo interval for the board’s difference: ±{interval:.1f} points (approximately 95%). This measures simulation sampling noise, not uncertainty in projections or model assumptions. Weekly substitutions and bench coverage are not scored.</p></div></div></section>'''
    replace_section(ROOT/'docs/archive/v2-site.html','benchmark',fragment)
    ...
    readme=ROOT/'docs/archive/v2-benchmark.md'
