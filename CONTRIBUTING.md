...
4. **Simulator improvements** in `scripts/draft_sim.py` — better opponent models, a better rest-of-draft policy inside the rollouts (the weakest link: a mediocre policy shrinks the differences between candidates), coverage modeling and supported keeper handling. Keep it standard-library plus PyYAML, keep the `sim.json` schema backward-compatible, and run the sample league before and after (`examples/sample-league/`) so the change is visible.
...
## v3 contribution checks

New providers need executable synthetic fixtures, explicit independence and freshness, canonical ID matching, completeness checks and an honest outage path. Authorized user imports are welcome; basic use must not require paid credentials. Do not commit licensed exports or private league sessions.

Run `python3 -m unittest discover -s tests -v`, then `python3 scripts/package.py`. Tests exercise the distributed ZIP outside the checkout. Keep source and download aligned. Versioned receipts and snapshots are public contracts: reject unsupported versions and test any migration. Separate score changes from evidence of decision quality; improve held-out evaluation before advertising better results.

Future lineup and waiver skills should reuse the core evidence, identity and scoring contracts while adding their own lock-time and transaction rules. See `docs/toolkit-architecture.md`. Do not introduce a universal score shared by unrelated fantasy decisions.
